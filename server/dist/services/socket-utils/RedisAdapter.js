...
function adapter(uri, opts) {
  ...
  // handle options only
  if ('object' == (typeof uri === 'undefined' ? 'undefined' : _typeof(uri))) {
    opts = uri;
    uri = null;
  }
  ...
  Redis.prototype.usersExists = function (user, socketId) {
    for (var key in this.nsp.connected) {
      if (this.nsp.connected.hasOwnProperty(key)) {
        console.log('aici', key, socketId);
        var connectedUser = this.nsp.connected[key].user;
        //console.log('aici2', connectedUser._id, user._id);
        if (connectedUser && connectedUser._id === user._id && socketId != key) {
          console.log('EXISTS DUPLICATE');
        }
      }
    }
  };
